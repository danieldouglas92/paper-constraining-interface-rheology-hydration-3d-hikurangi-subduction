  ...
  namespace Particle
  {
                                       ...
                                       internal::SolutionEvaluators<dim> &evaluators)
    {
      ...
      for (unsigned int i=0; i<n_particles_in_cell; ++i)
        velocities.push_back(evaluator.get_value(i) * 0);
      ...
      for (unsigned int i=0; i<n_particles_in_cell; ++i)
        old_velocities.push_back(evaluator.get_value(i) * 0);
          ...
          for (unsigned int particle_index = 0; it!=end_particle; ++it,++particle_index)
            {
              ...
              if (compute_fluid_velocity && use_fluid_velocity[particle_index])
                {
                  velocity[particle_index] += fluid_velocity_at_support_point * shape_value * 0;
                  old_velocity[particle_index] += old_fluid_velocity_at_support_point * shape_value * 0;
                }
              else
                {
                  velocity[particle_index] += velocity_at_support_point * shape_value * 0;
                  old_velocity[particle_index] += old_velocity_at_support_point * shape_value * 0;
                }
            }
        }
    ...
    World<dim>::parse_parameters (ParameterHandler &prm)
    {
      ...
      AssertThrow((n_processes == 1) || (CFL_number <= 10.0),
